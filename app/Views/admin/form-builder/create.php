...
<div class="space-y-6 max-w-2xl mx-auto">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-xl font-bold text-slate-800 dark:text-slate-100">Tambah Template Formulir</h1>
            <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Definisikan formulir dinamis baru untuk mengisi data kuesioner/registrasi.</p>
        </div>
    </div>

    <!-- Form Card -->
    <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6 shadow-sm">
        <form action="<?= base_url('admin/form-builder/store') ?>" method="POST" class="space-y-5 text-xs">
            <?= csrf_field() ?>

            <!-- Field: name -->
            <div>
                <label class="block font-semibold text-slate-700 dark:text-slate-300">Nama Formulir <span class="text-red-500">*</span></label>
                <input type="text" name="name" value="<?= old('name') ?>" placeholder="Contoh: Form Evaluasi Kinerja Mahasiswa oleh Mitra" class="block w-full mt-1.5 px-3 py-2 bg-white dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800 rounded-lg text-slate-850 dark:text-slate-100 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/20 <?= isset(session('errors')['name']) ? 'border-red-500' : '' ?>" required />
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="form_type" class="block w-full mt-1.5 px-3 py-2 bg-white dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800 rounded-lg text-slate-850 dark:text-slate-100 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/20" required>
                        <option value="registration" <?= old('form_type') === 'registration' ? 'selected' : '' ?>>Pendaftaran (Registrasi)</option>
                        <option value="logbook" <?= old('form_type') === 'logbook' ? 'selected' : '' ?>>Logbook Mingguan</option>
                        <option value="assessment" <?= old('form_type') === 'assessment' ? 'selected' : '' ?>>Penilaian Akhir</option>
                        <option value="general" <?= old('form_type') === 'general' ? 'selected' : '' ?>>Umum / Lainnya</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="period_id" class="block w-full mt-1.5 px-3 py-2 bg-white dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800 rounded-lg text-slate-850 dark:text-slate-100 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/20">
                        ...
                        <?php foreach ($periods as $p): ?>
                            <option value="<?= $p['id'] ?>" <?= old('period_id') == $p['id'] ? 'selected' : '' ?>><?= esc($p['name']) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
