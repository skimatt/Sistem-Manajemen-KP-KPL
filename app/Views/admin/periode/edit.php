...
<div class="space-y-6 max-w-2xl mx-auto">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-xl font-bold text-slate-800 dark:text-slate-100">Edit Periode</h1>
            <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Ubah rincian periode akademik: <?= esc($period['name']) ?>.</p>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6 shadow-sm">
        <form action="<?= base_url('admin/periode/update/' . $period['id']) ?>" method="POST" class="space-y-5">
            <?= csrf_field() ?>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select name="study_program_id" 
                            ...
                            <?php foreach ($prodis as $prodi): ?>
                                <option value="<?= $prodi['id'] ?>" <?= old('study_program_id', $period['study_program_id']) == $prodi['id'] ? 'selected' : '' ?>>
                    ...
                    <div>
                        ...
                        <select name="activity_type" 
                            ...
                            <option value="KP" <?= old('activity_type', $period['activity_type']) === 'KP' ? 'selected' : '' ?>>KP (Kerja Praktek)</option>
                            <option value="KPL" <?= old('activity_type', $period['activity_type']) === 'KPL' ? 'selected' : '' ?>>KPL (Kerja Praktek Lapangan)</option>
                        </select>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="date" 
                               ...
                               value="<?= old('registration_start', $period['registration_start']) ?>" 
                               class="block w-full mt-1.5 px-3 py-2 text-xs bg-white dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800 rounded-lg text-slate-850 dark:text-slate-100 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/20" 
                               required />
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" 
                               ...
                               value="<?= old('registration_end', $period['registration_end']) ?>" 
                               class="block w-full mt-1.5 px-3 py-2 text-xs bg-white dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800 rounded-lg text-slate-850 dark:text-slate-100 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/20" 
                               required />
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" 
                               name="activity_start" 
                               value="<?= old('activity_start', $period['activity_start']) ?>" 
                               class="block w-full mt-1.5 px-3 py-2 text-xs bg-white dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800 rounded-lg text-slate-850 dark:text-slate-100 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/20" 
                               required />
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" 
                               name="activity_end" 
                               value="<?= old('activity_end', $period['activity_end']) ?>" 
                               class="block w-full mt-1.5 px-3 py-2 text-xs bg-white dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800 rounded-lg text-slate-850 dark:text-slate-100 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/20" 
                               required />
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="status" 
                            ...
                            <option value="ditutup" <?= old('status', $period['status']) === 'ditutup' ? 'selected' : '' ?>>Ditutup</option>
                            <option value="diarsipkan" <?= old('status', $period['status']) === 'diarsipkan' ? 'selected' : '' ?>>Diarsipkan</option>
                        </select>
                        <span class="text-[10px] text-slate-400 dark:text-slate-500 mt-1 block">Catatan: Mengaktifkan periode ini akan menonaktifkan periode aktif lain pada program studi & tipe kegiatan yang sama.</span>
                    </div>
                </div>
            </div>
