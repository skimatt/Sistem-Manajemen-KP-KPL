...
<div class="space-y-6">
    <!-- Header Section -->
    <div>
        <h1 class="text-xl font-bold text-slate-800 dark:text-slate-100">Verifikasi Administrasi</h1>
        <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Audit dan periksa berkas kelengkapan persyaratan pendaftaran KP/KPL mahasiswa.</p>
    </div>
    ...
    <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6 shadow-sm">
        <div class="overflow-x-auto">
            <table id="verifTable" class="w-full text-xs text-left text-slate-600 dark:text-slate-350">
                <thead>
                    <tr class="text-slate-400 dark:text-slate-500 border-b border-slate-200 dark:border-slate-800 uppercase text-[9px] tracking-wider font-semibold">
                        ...
                        <th class="pb-3">Uploaded Dokumen</th>
                        <th class="pb-3">Belum Diverifikasi</th>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800/80">
                    <?php foreach ($students as $student): ?>
                        <tr>
                            ...
                            <td class="py-3.5 text-slate-650 dark:text-slate-300 font-semibold font-mono"><?= esc($student['total_uploaded']) ?> Berkas</td>
                            <td class="py-3.5">
                                <?php if ($student['pending_count'] > 0): ?>
                                    <span class="inline-flex items-center gap-1.5 px-2 py-0.5 rounded text-[10px] font-bold bg-amber-50 dark:bg-amber-950/40 text-amber-700 dark:text-amber-400 border border-amber-100 dark:border-amber-900/40 animate-pulse">
                                        <?= esc($student['pending_count']) ?> Menunggu
                                    </span>
                                <?php else: ?>
                                    <span class="inline-flex items-center gap-1.5 px-2 py-0.5 rounded text-[10px] font-bold bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-400 border border-emerald-100 dark:border-emerald-900/40">
                                        Clear
                                    </span>
                                <?php endif; ?>
                            </td>
                            <td class="py-3.5 text-right">
                                <a href="<?= base_url('admin/verifikasi-administrasi/review/' . $student['registration_id']) ?>" class="inline-flex items-center gap-1 px-2.5 py-1 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-md transition text-[10px] font-bold" title="Audit">
                                    <i class="ti ti-checklist text-sm"></i> Periksa Berkas
                                </a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
