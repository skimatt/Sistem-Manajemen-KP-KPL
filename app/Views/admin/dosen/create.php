...
<div class="space-y-6 max-w-2xl mx-auto">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-xl font-bold text-slate-800 dark:text-slate-100">Tambah Dosen</h1>
            <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Daftarkan profil dosen pembimbing baru dan buatkan akun loginnya secara bersamaan.</p>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6 shadow-sm">
        <form action="<?= base_url('admin/dosen/store') ?>" method="POST" class="space-y-5">
            <?= csrf_field() ?>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="number" 
                               ...
                               value="<?= old('max_supervision_quota', 10) ?>" 
                    ...
                    <div>
                        ...
                        <select name="is_available" 
                                class="block w-full mt-1.5 px-3 py-2 text-xs bg-white dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800 rounded-lg text-slate-850 dark:text-slate-100 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/20" 
                                required>
                            <option value="1" <?= old('is_available', '1') === '1' ? 'selected' : '' ?>>Tersedia (Bisa Menerima Bimbingan)</option>
                            <option value="0" <?= old('is_available') === '0' ? 'selected' : '' ?>>Penuh (Tidak Menerima Bimbingan)</option>
                        </select>
                    </div>
                </div>
            </div>
