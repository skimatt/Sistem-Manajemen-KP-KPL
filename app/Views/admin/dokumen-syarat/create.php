...
<div class="space-y-6 max-w-2xl mx-auto">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-xl font-bold text-slate-800 dark:text-slate-100">Tambah Dokumen Persyaratan</h1>
            <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Definisikan dokumen wajib/opsional baru untuk periode tertentu.</p>
        </div>
    </div>

    <!-- Form Card -->
    <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6 shadow-sm">
        <form action="<?= base_url('admin/dokumen-syarat/store') ?>" method="POST" class="space-y-5 text-xs">
            <?= csrf_field() ?>
            ...
            <div>
                ...
                <select name="period_id" class="block w-full mt-1.5 px-3 py-2 bg-white dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800 rounded-lg text-slate-850 dark:text-slate-100 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/20 <?= isset(session('errors')['period_id']) ? 'border-red-500' : '' ?>" required>
                    ...
                    <?php foreach ($periods as $p): ?>
                        <option value="<?= $p['id'] ?>" <?= old('period_id') == $p['id'] ? 'selected' : '' ?>><?= esc($p['name']) ?> (<?= esc(str_replace('_', ' ', $p['status'])) ?>)</option>
                    <?php endforeach; ?>
                </select>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                    ...
                    <select name="is_required" class="block w-full mt-1.5 px-3 py-2 bg-white dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800 rounded-lg text-slate-850 dark:text-slate-100 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/20" required>
                        <option value="1" <?= old('is_required', 1) == 1 ? 'selected' : '' ?>>Wajib Diisi</option>
                ...
                <div>
                    ...
                    <select name="stage" class="block w-full mt-1.5 px-3 py-2 bg-white dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800 rounded-lg text-slate-850 dark:text-slate-100 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/20" required>
                        <option value="registrasi" <?= old('stage') === 'registrasi' ? 'selected' : '' ?>>Registrasi Awal</option>
                        <option value="penempatan" <?= old('stage') === 'penempatan' ? 'selected' : '' ?>>Penempatan</option>
                        <option value="penilaian" <?= old('stage') === 'penilaian' ? 'selected' : '' ?>>Penilaian</option>
                    </select>
                </div>

                <div>
                    <label class="block font-semibold text-slate-700 dark:text-slate-300">Urutan Tampil <span class="text-red-500">*</span></label>
                    <input type="number" name="sort_order" value="<?= old('sort_order', 0) ?>" class="block w-full mt-1.5 px-3 py-2 bg-white dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800 rounded-lg text-slate-850 dark:text-slate-100 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/20" required />
                </div>
            </div>
            ...
            <div>
                ...
                <select name="status" class="block w-full mt-1.5 px-3 py-2 bg-white dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800 rounded-lg text-slate-850 dark:text-slate-100 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/20" required>
                    <option value="active" <?= old('status', 'active') === 'active' ? 'selected' : '' ?>>Aktif (Tampil)</option>
            ...
            <div class="flex items-center justify-end gap-3 pt-3 border-t border-slate-100 dark:border-slate-800">
                <a href="<?= base_url('admin/dokumen-syarat') ?>" class="px-4 py-2 border border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400 hover:text-slate-750 dark:hover:text-slate-200 rounded-lg font-semibold transition">
                    Batal
                </a>
