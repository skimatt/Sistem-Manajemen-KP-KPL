...
<div class="space-y-6 max-w-2xl mx-auto" x-data="{ hasAccount: <?= old('has_account') === '1' ? 'true' : 'false' ?> }">
    <!-- Header Section -->
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-xl font-bold text-slate-800 dark:text-slate-100">Tambah Instansi</h1>
            <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Daftarkan profil instansi mitra resmi baru atau pengajuan mandiri.</p>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6 shadow-sm">
        <form action="<?= base_url('admin/instansi/store') ?>" method="POST" class="space-y-5">
            <?= csrf_field() ?>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select name="type" 
                                class="block w-full mt-1.5 px-3 py-2 text-xs bg-white dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800 rounded-lg text-slate-850 dark:text-slate-100 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/20" 
                                required>
                            <option value="mitra" <?= old('type') === 'mitra' ? 'selected' : '' ?>>Mitra Kampus (Resmi)</option>
                            <option value="mandiri" <?= old('type') === 'mandiri' ? 'selected' : '' ?>>Mandiri (Usulan Mahasiswa)</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="partnership_status" 
                                class="block w-full mt-1.5 px-3 py-2 text-xs bg-white dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800 rounded-lg text-slate-850 dark:text-slate-100 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/20" 
                                required>
                            <option value="active" <?= old('partnership_status') === 'active' ? 'selected' : '' ?>>Aktif (Kerja Sama Terjalin)</option>
                            <option value="pending" <?= old('partnership_status') === 'pending' ? 'selected' : '' ?>>Pending (Dalam Proses Pengajuan)</option>
                            <option value="inactive" <?= old('partnership_status') === 'inactive' ? 'selected' : '' ?>>Nonaktif</option>
                        </select>
                    </div>
                </div>
            </div>
            ...
            <div>
                <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800/80 pb-2 mb-4">
                    ...
                    <div>
                        <select name="has_account" 
                            ...
                            <option value="0" <?= old('has_account') === '0' ? 'selected' : '' ?>>Nonaktifkan Akun</option>
                            <option value="1" <?= old('has_account') === '1' ? 'selected' : '' ?>>Aktifkan Akun</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4" x-show="hasAccount" x-transition x-cloak>
                    ...
                    <!-- Password -->
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 dark:text-slate-350">Password Akun <span class="text-red-550">*</span></label>
                        <input type="password" 
                               name="password" 
