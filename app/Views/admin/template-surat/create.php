...
<div class="space-y-6 max-w-4xl mx-auto">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-xl font-bold text-slate-800 dark:text-slate-100">Tambah Template Surat</h1>
            <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Definisikan template dokumen HTML baru beserta placeholder variabel dinamis.</p>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6 shadow-sm">
        <form action="<?= base_url('admin/template-surat/store') ?>" method="POST" class="space-y-5 text-xs">
            <?= csrf_field() ?>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                    ...
                    <select name="document_type" class="block w-full mt-1.5 px-3 py-2 bg-white dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800 rounded-lg text-slate-850 dark:text-slate-100 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/20" required>
                        <option value="rekomendasi_pa" <?= old('document_type') === 'rekomendasi_pa' ? 'selected' : '' ?>>Surat Rekomendasi PA</option>
                        <option value="permohonan_mitra" <?= old('document_type') === 'permohonan_mitra' ? 'selected' : '' ?>>Surat Permohonan Mitra</option>
                        <option value="tugas_pembimbing" <?= old('document_type') === 'tugas_pembimbing' ? 'selected' : '' ?>>Surat Tugas Pembimbing</option>
                        <option value="rekap_nilai" <?= old('document_type') === 'rekap_nilai' ? 'selected' : '' ?>>Rekap Nilai Akhir</option>
                ...
                <div>
                    ...
                    <input type="number" name="version" value="<?= old('version', 1) ?>" class="block w-full mt-1.5 px-3 py-2 bg-white dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800 rounded-lg text-slate-850 dark:text-slate-100 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/20" required />
                </div>
            ...
            <div>
                ...
                <textarea name="content_html" rows="12" placeholder="Tuliskan struktur HTML..." class="block w-full mt-1.5 px-3 py-2 font-mono bg-white dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800 rounded-lg text-slate-850 dark:text-slate-100 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/20 <?= isset(session('errors')['content_html']) ? 'border-red-500' : '' ?>" required><?= old('content_html', "<h2>SURAT TUGAS KP/KPL</h2>\n<p>Yang bertanda tangan di bawah ini menerangkan bahwa:</p>\n<p>Nama: {nama_mahasiswa}<br>NPM: {npm}<br>Program Studi: {prodi}</p>\n<p>Ditugaskan melaksanakan kegiatan pada {nama_instansi} periode {periode}.</p>") ?></textarea>
