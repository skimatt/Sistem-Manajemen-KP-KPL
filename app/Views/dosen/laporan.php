...
<div class="space-y-6">
    <!-- Header -->
    <div>
        <h1 class="text-xl font-bold text-slate-800 dark:text-slate-100">Persetujuan Laporan Akhir</h1>
        <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Review berkas laporan akhir KP/KPL yang diunggah oleh mahasiswa bimbingan Anda.</p>
    </div>
    ...
    <div class="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-xl p-4 shadow-sm transition-colors duration-300">
        <div class="overflow-x-auto">
            <table id="reportsTable" class="w-full text-xs text-left text-slate-600 dark:text-slate-400">
                <thead>
                    <tr class="text-slate-400 dark:text-slate-500 border-b border-slate-100 dark:border-slate-800 uppercase text-[9px] tracking-wider">
                        ...
                        <th class="py-3 px-4 font-semibold">Versi Berkas</th>
                        <th class="py-3 px-4 font-semibold">Status Review</th>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800/60">
                    <?php foreach ($reports as $r): ?>
                        <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-colors duration-150">
                            ...
                            <td class="py-3 px-4 text-slate-800 dark:text-slate-350 max-w-xs truncate font-medium">
                                <?= esc($r['title']) ?>
                            </td>
                            <td class="py-3 px-4 text-slate-500 dark:text-slate-400 font-bold">
                                v<?= esc($r['version']) ?>
                            </td>
                            <td class="py-3 px-4">
                                <?php 
                            ...
                            <td class="py-3 px-4 text-right">
                                <?php if ($status === 'dikirim'): ?>
                                    <a id="action_check_report_<?= esc($r['id']) ?>" href="<?= base_url('dosen/laporan/review/' . $r['id']) ?>" class="inline-flex items-center gap-1 px-2.5 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-[10px] font-semibold shadow shadow-blue-500/10 transition">
                                        <i class="ti ti-edit"></i> Periksa
                                    </a>
                                <?php else: ?>
                                    <a id="action_view_report_<?= esc($r['id']) ?>" href="<?= base_url('dosen/laporan/review/' . $r['id']) ?>" class="inline-flex items-center gap-1 px-2.5 py-1.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 rounded-lg text-[10px] font-semibold transition">
