...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-xl p-4 flex items-center gap-4 shadow-sm transition-colors duration-300">
            ...
            <div>
                <p class="text-[10px] font-semibold text-slate-400 dark:text-slate-505 uppercase tracking-wider">Mahasiswa Bimbingan</p>
                <p class="text-sm font-bold text-slate-800 dark:text-slate-200 mt-0.5">
                    <?= esc($stats['total_students']) ?> <span class="text-xs text-slate-400 dark:text-slate-500 font-normal">/ <?= esc($stats['max_quota']) ?></span>
                </p>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-xl p-4 flex items-center gap-4 shadow-sm transition-colors duration-300">
            ...
            <div>
                <p class="text-[10px] font-semibold text-slate-400 dark:text-slate-550 uppercase tracking-wider">Logbook Perlu Review</p>
        ...
        <div class="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-xl p-4 flex items-center gap-4 shadow-sm transition-colors duration-300">
            ...
            <div>
                <p class="text-[10px] font-semibold text-slate-400 dark:text-slate-550 uppercase tracking-wider">Laporan Perlu Review</p>
        ...
        <div class="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-xl p-4 flex items-center gap-4 shadow-sm transition-colors duration-300">
            ...
            <div>
                <p class="text-[10px] font-semibold text-slate-400 dark:text-slate-555 uppercase tracking-wider">Kuota Tersedia</p>
                <p class="text-lg font-bold text-slate-800 dark:text-slate-200 mt-0.5 font-mono">
    ...
    <div class="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-xl p-4 shadow-sm transition-colors duration-300">
        <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3 mb-4">
            <h2 class="text-sm font-bold text-slate-800 dark:text-slate-200 flex items-center gap-1.5">
                <i class="ti ti-users text-slate-400 dark:text-slate-500"></i> Daftar Mahasiswa Bimbingan Aktif
            </h2>
            <a href="<?= base_url('dosen/mahasiswa') ?>" class="text-[11px] font-semibold text-blue-600 dark:text-blue-400 hover:text-blue-500">Lihat Semua</a>
        </div>
            ...
            <table class="w-full text-xs text-left text-slate-600 dark:text-slate-400">
                <thead>
                    <tr class="text-slate-400 dark:text-slate-500 border-b border-slate-100 dark:border-slate-800 uppercase text-[9px] tracking-wider">
                        ...
                        <th class="py-2.5 font-semibold">Periode</th>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800/60">
                    <?php if (empty($students)): ?>
                        <tr>
                            <td colspan="5" class="py-6 text-center text-slate-400 dark:text-slate-500">Belum ada mahasiswa yang didelegasikan ke Anda.</td>
                        </tr>
                    <?php else: ?>
                        <?php foreach ($students as $student): ?>
                            <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/25 transition">
                                ...
                                <td class="py-3 text-slate-500 dark:text-slate-400"><?= esc($student['period_name']) ?></td>
                                <td class="py-3">
                                    <?php 
