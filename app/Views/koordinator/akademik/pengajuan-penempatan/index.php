...
<div class="space-y-6 max-w-7xl mx-auto">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
            <h1 class="text-xl font-bold text-slate-800 dark:text-slate-100">Pengajuan Penempatan Mitra Kampus</h1>
            <p class="text-xs text-slate-400 dark:text-slate-500 mt-1">Review pengajuan penempatan mahasiswa pada instansi mitra kampus.</p>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800 rounded-xl overflow-hidden shadow-sm">
        <div class="p-6">
            <div class="table-responsive">
                <table id="pengajuanPenempatanTable" class="w-full text-xs text-left text-slate-600 dark:text-slate-400">
                    <thead class="text-[10px] uppercase bg-slate-50 dark:bg-slate-800/50 text-slate-500 dark:text-slate-400 border-b border-slate-200/60 dark:border-slate-850">
                        <tr>
                            ...
                            <th class="px-6 py-3.5 font-bold">Instansi Mitra</th>
                            <th class="px-6 py-3.5 font-bold">Periode</th>
                            <th class="px-6 py-3.5 font-bold">Tanggal Pengajuan</th>
                            ...
                            <?php foreach ($placements as $place): ?>
                                <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-850/30 transition duration-150">
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        <a href="<?= base_url('koordinator/pengajuan-penempatan/review/' . $place['id']) ?>" 
...
<script>
    $(document).ready(function() {
        $('#pengajuanPenempatanTable').DataTable({
            language: {
